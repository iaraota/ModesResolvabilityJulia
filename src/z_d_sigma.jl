using DelimitedFiles, Distributed, HDF5, Random, Distributions
...
function RunAllSXSFolders(masses, detector, F_Re, F_Im, N_avg, label)
            ...
            for mode_2 in modes
                for M_f in masses
                    for num_par in nums
						...
						save_dict[save_dir] = zeros(Int(N), 9)
                    	Base.Threads.@threads for i in 1:Int(N)
