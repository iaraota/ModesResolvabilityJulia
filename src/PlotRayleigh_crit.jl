...
function ContourPoints(X,Y,Z)
	...
	aux = 0
	while aux == 0
		if y[i_aux] <= y[i_aux + 1] && i_aux < length(y) - 1
			i_right = i_aux
			i_aux += 1
		else
			aux = 1
...
function NewDataNumberResolvable(detector, q_mass = 1.5)
	...
	for num_pars in ["4", "6"]
		NumResolv = Dict()
		X, Y, NumResolv[2], NumResolv[3], NumResolv[4], NumResolv[5] = NumberResolvableModes(DataModes, num_pars, "tau", "critical")
		h5open("data/rayleigh/NumberModeHorizon_"*label*"_crit.h5", "cw") do file
		...
		for (key, value) in DataModes
			...
			XX = value[num_pars][1]
			YY = value[num_pars][2]
			ResolveData[key] = value[num_pars][7]
		end
		...
		h5open("data/rayleigh/ModeHorizon_"*label*"_crit.h5", "cw") do file
...
function NewDataAllDetectorsRayleigh(q_mass)
	...
	if isfile("data/rayleigh/NumberModeHorizon_"*label*"_crit.h5")
		rm("data/rayleigh/NumberModeHorizon_"*label*"_crit.h5")
	end

	if isfile("data/rayleigh/ModeHorizon_"*label*"_crit.h5")
		rm("data/rayleigh/ModeHorizon_"*label*"_crit.h5")
	end
...
function PlotResolvableModesContourAll(q_mass)
	...
	DataNResolv = h5open("data/rayleigh/NumberModeHorizon_"*label*"_crit.h5") do file
		read(file) 
	end
...
function PlotHorizon2modesContourRayleigh(q_mass, choose_modes)
	...
	DataModes = h5open("data/rayleigh/ModeHorizon_"*label*"_crit.h5") do file
		read(file) 
	end
	...
	for num_pars in ["4", "6"]
		...
		plt.savefig("figs/rayleigh/"*detector*"_"*choose_modes*"_"*num_pars*"_"*label*"_crit.pdf")
	end
end
...
function PlotHorizon2modesContourRayleighAllDetectors(q_mass, choose_modes)
	...
	for num_pars in ["4", "6"]
		...
		plt.savefig("figs/rayleigh/AllDetectors_"*choose_modes*"_"*num_pars*"_"*label*"_crit.pdf")
	end
end
